...
static LPCWSTR tls_server_end_point_algorithm(PCCERT_CONTEXT cert) {
  const char *oid = cert->pCertInfo->SignatureAlgorithm.pszObjId;
  if (!oid) {
...
static int32_t hash_bytes(
  ...
  status = BCryptOpenAlgorithmProvider(&algorithm, algorithm_name, NULL, 0);
  ...
  status = BCryptHashData(hasher, (PUCHAR)data, data_len, 0);
  if (status != STATUS_SUCCESS)
    goto exit_with_hasher;

  status = BCryptFinishHash(hasher, out, digest_len, 0);
...
exit:
  ...
  return (int32_t)digest_len;
}

MOONBIT_FFI_EXPORT
void moonbit_postgres_client_tls_schannel_free(struct Context *ctx) {
  switch (ctx->state) {
...
MOONBIT_FFI_EXPORT
struct Context *moonbit_postgres_client_tls_schannel_new() {
...
MOONBIT_FFI_EXPORT
int32_t moonbit_postgres_client_tls_schannel_bytes_read(struct Context *ctx) {
  return ctx->bytes_read;
}

MOONBIT_FFI_EXPORT
int32_t moonbit_postgres_client_tls_schannel_bytes_to_write(struct Context *ctx) {
  return ctx->bytes_to_write;
}

MOONBIT_FFI_EXPORT
int32_t moonbit_postgres_client_tls_schannel_msg_trailer(struct Context *ctx) {
  return ctx->msg_trailer;
}

MOONBIT_FFI_EXPORT
int32_t moonbit_postgres_client_tls_schannel_header_size(struct Context *ctx) {
  return ctx->stream_sizes.cbHeader;
}

MOONBIT_FFI_EXPORT
int32_t moonbit_postgres_client_tls_schannel_trailer_size(struct Context *ctx) {
  return ctx->stream_sizes.cbTrailer;
}

MOONBIT_FFI_EXPORT
int32_t moonbit_postgres_client_tls_schannel_init_client(
  struct Context *ctx,
  int32_t verify
) {
...
MOONBIT_FFI_EXPORT
int32_t moonbit_postgres_client_tls_schannel_init_server(struct Context *ctx) {
...
MOONBIT_FFI_EXPORT
int32_t moonbit_postgres_client_tls_schannel_connect(
  struct Context *ctx,
  LPWSTR host_name,
  char *in_buffer,
...
MOONBIT_FFI_EXPORT
int32_t moonbit_postgres_client_tls_schannel_accept(
  struct Context *ctx,
  char *in_buffer,
  int32_t in_buffer_offset,
  int32_t in_buffer_len,
...
MOONBIT_FFI_EXPORT
int32_t moonbit_postgres_client_tls_schannel_read(
  struct Context *ctx,
  char *buffer,
  int32_t offset,
  int32_t len
...
MOONBIT_FFI_EXPORT
int32_t moonbit_postgres_client_tls_schannel_write(
  struct Context *ctx,
  char *buffer,
  int32_t offset,
  int32_t len
...
MOONBIT_FFI_EXPORT
int32_t moonbit_postgres_client_tls_schannel_shutdown(struct Context *ctx) {
...
MOONBIT_FFI_EXPORT
int32_t moonbit_postgres_client_tls_schannel_tls_server_end_point(
  struct Context *ctx,
  unsigned char *out
) {
  if (ctx->state != ContextInitialized) {
    SetLastError(SEC_E_INVALID_HANDLE);
    return -1;
  }
  ...
  int32_t len = hash_bytes(
    tls_server_end_point_algorithm(cert),
    cert->pbCertEncoded,
    cert->cbCertEncoded,
    out,
    64
...
MOONBIT_FFI_EXPORT
int32_t moonbit_postgres_client_tls_rand_bytes(void *buf, int32_t num) {
  return BCryptGenRandom(NULL, buf, num, BCRYPT_USE_SYSTEM_PREFERRED_RNG) == STATUS_SUCCESS;
}

MOONBIT_FFI_EXPORT
int32_t moonbit_postgres_client_tls_SHA1(void *data, int32_t num, void *out) {
